...
#if defined(CONFIG_GNSS_SAMPLE_MODE_TTFF_TEST)
	// k_work_schedule_for_queue(&gnss_work_q, &ttff_test_prepare_work, K_NO_WAIT); // CHANGE
...
static void print_fix_data(struct nrf_modem_gnss_pvt_data_frame *pvt_data)
{
	...
	len = snprintf(msg, sizeof(msg), "OK,%.06f,%.06f,%.01f,%.01f,%.01f,%.01f\n", pvt_data->latitude,
																			pvt_data->longitude,
																			(double)pvt_data->altitude,
																			(double)pvt_data->accuracy,
																			(double)pvt_data->pdop,
...
int main(void)
{
	...
	for (;;) {
		...
		if (events[2].state == K_POLL_STATE_MSGQ_DATA_AVAILABLE && 
			0 == uart_wait_for_msg(rx_buffer, sizeof(rx_buffer), K_NO_WAIT))
		{
			if (0 == strncmp("START", rx_buffer, 5))
			{
				k_work_schedule_for_queue(&gnss_work_q, &ttff_test_prepare_work, K_NO_WAIT);
			}
			else if (0 == strncmp("STOP", rx_buffer, 4))
			{
				k_work_cancel_delayable(&gnss_work_q);
				nrf_modem_gnss_stop();
			}
			else
				printf("Wrong msg: %s\n", rx_buffer);
